...
fun rainbow#hi(config)
	...
	for id in range(maxlvl)
		...
		let ctermbg = conf.ctermbgs[id % len(conf.ctermbgs)]
		let ctermfg = conf.ctermfgs[id % len(conf.ctermfgs)]
		let guibg = conf.guibgs[id % len(conf.guibgs)]
		let guifg = conf.guifgs[id % len(conf.guifgs)]
		exe 'hi '.prefix.'_p'.id.' cterm='.cterm.' ctermfg='.ctermfg.' ctermbg='.ctermbg.' guifg='.guifg.' guibg='.guibg
		exe 'hi '.prefix.'_o'.id.' cterm='.cterm.' ctermfg='.ctermfg.' ctermbg='.ctermbg.' guifg='.guifg.' guibg='.guibg
	endfor
endfun
